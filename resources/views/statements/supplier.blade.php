...
<head>
    ...
    <title>Relevé Fournisseur — {{ $supplier->name }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status-received,.status-approved { color: #d97706; font-weight: 700; }
...
<div class="page">
    ...
    @if($company->tax_center && str_contains(strtolower($company->tax_center), 'dge') || str_contains(strtolower($company->tax_center ?? ''), 'cime'))
    <div class="note">
        ⚠ Régime DGE/CIME — Précompte de 5,5% applicable sur les achats. Le montant « Net à Payer » est net de précompte.
    </div>
    @endif
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="num" style="color:#d97706">{{ number_format($inv->withholding_amount, 0, ',', ' ') }} <span class="xaf">XAF</span></td>
                <td class="num"><strong>{{ number_format($inv->net_payable, 0, ',', ' ') }}</strong> <span class="xaf">XAF</span></td>
                <td class="status-{{ strtolower($inv->status) }}">{{ $inv->status }}</td>
            </tr>
            @empty
            <tr><td colspan="8" style="text-align:center;padding:16px;color:#94a3b8">Aucune facture sur cette période.</td></tr>
            @endforelse
        </tbody>
    </table>

    <div class="summary">
        <div class="summary-row"><span>Total Net à Payer</span><span>{{ number_format($total_due, 0, ',', ' ') }} XAF</span></div>
        <div class="summary-row"><span>Total Réglé</span><span style="color:#16a34a">{{ number_format($total_paid, 0, ',', ' ') }} XAF</span></div>
        <div class="summary-row total"><span>Solde Restant Dû</span><span style="color:#dc2626">{{ number_format($balance, 0, ',', ' ') }} XAF</span></div>
    </div>
